...
<script>
  // Flikarna styrs av adressens hash (#om, #kallor, ...) så att varje flik –
  // och varje rubrik inuti en flik – går att länka till direkt.
  ...
  function applyHash(smooth) {{
    ...
    if (!id) {{
      activatePanel('analysen');
      return;
    }}
    // Hashen pekar antingen på en flik ...
    if (activatePanel(id)) {{
      window.scrollTo({{ top: 0, behavior: smooth ? 'smooth' : 'auto' }});
      return;
    }}
    // ... eller på ett avsnitt inuti en flik, som då öppnas först.
    var target = document.getElementById(id);
    if (!target) return;
    var panel = target.closest('.panel');
    if (panel) activatePanel(panel.id);
  ...
  document.querySelectorAll('.tab').forEach(function (tab) {{
    tab.addEventListener('click', function () {{
      ...
      if (location.hash.slice(1) === name) applyHash(true);
